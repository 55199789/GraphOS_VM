...
void ecall_set_new_minheap_node(int newMinHeapNodeV, int newMinHeapNodeDist) {
    ...
    std::cout<<"newMinHeapNodeV: "<<newMinHeapNodeV <<", newMinHeapNodeDist: " << newMinHeapNodeDist<<std::endl;
...
void ecall_write_node(const char *bid, const char* value) {
    if (setup) {
        ...
        Bid inputBid(id);
        string val(value);
        std::cout << "inserting omap" << std::endl;
...
double ecall_measure_oram_speed(int testSize) {
    ...
    ORAM* oram = new ORAM(testSize, false);
    ...
    for (int j = 0; j < 5; j++) {
        ...
        for (int i = 1; i <= 500; i++) {
            ...
            oram->start(false);
            oram->ReadWrite(id, node, pos, pos, false, false, false);
            time1 = ocall_stop_timer(535);
            ocall_start_timer(535);
            oram->start(false);
            Node* tmpNode = NULL;
            Node* res = oram->ReadWrite(id, dummyNode, readpos, readpos, true, false, false);
            time2 = ocall_stop_timer(535);
            string resStr = "";
            resStr.assign(res->value.begin(), res->value.end());
            resStr = resStr.c_str();
            delete res;
...
double ecall_measure_eviction_speed(int testSize) {
    ...
    ORAM* oram = new ORAM(s, true, true);
...
double ecall_measure_oram_setup_speed(int testSize) {
    ...
    double time2;
    ocall_start_timer(535);
    ORAM* oram = new ORAM(maxSize, &nodes, permutation);
    time2 = ocall_stop_timer(535);
...
double ecall_measure_omap_setup_speed(int testSize) {
    ...
    double time2;
    ocall_start_timer(535);
    OMAP* omap = new OMAP(maxSize, &pairs, &permutation);
    time2 = ocall_stop_timer(535);
    Bid testKey = 3;
